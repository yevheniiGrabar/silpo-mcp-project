...
class ProductMatchingService
{
    ...
    private function categoryProfile(?string $c): array
    {
        return match ($c) {
            ...
            'овочі' => [[], ['по-корейськ', 'маринован', 'консерв', 'сік ', 'чіпси', 'сушен', 'гранульован', 'порошок', 'приправ', 'спеці', 'kamis', 'пюре', 'швидк', 'чука', 'морськ', 'заморож', 'варен', 'смажен', 'запечен', 'фрі']],
        ...
        foreach (($data['queries'] ?? []) as $group) {
            ...
            foreach (($group['products'] ?? []) as $item) {
                ...
                $result[$query][] = new SilpoProduct(
                    ...
                    available: ($item['available'] ?? true) !== false && (float) ($item['stock'] ?? 1) > 0,
                    stock: (float) ($item['stock'] ?? 1),
                );
            }
        }
