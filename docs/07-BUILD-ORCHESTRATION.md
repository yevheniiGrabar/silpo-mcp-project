...
## НОВІ ФАЗИ (docs/09 — калорії, заміна страв, freemium-підписка)
- **B8 · Калорії/БЖУ** — `MealPlannerAgent` повертає ккал/БЖУ на страву (поля в MealPlanSchema); суми день/тиждень; (стретч) `NutritionService` через нутрієнт-базу. depends_on: B4 · джерело: `09 §1` · accept: кожна страва має калорії, суми рахуються, тест.
- **B9 · Заміна страви (live re-plan)** — `POST /meal-plans/{id}/meals/{meal}/swap` → `MealSwapService` (LLM 1 страва під ті самі обмеження → детермін. матчинг → перерахунок бюджету/калорій), не повторює альтернативи. depends_on: B5 · джерело: `09 §2` · accept: swap end-to-end, бюджет/калорії оновлюються.
- **B10 · Підписка/біллінг** — `plans`/`subscriptions`, провайдер (Creem/Stripe/Paddle або укр-еквайринг) + вебхуки; `PriceSource`-абстракція (Silpo=реалізація); gating платних фіч (middleware/policy `requiresPlan('pro')`) + free-ліміт. depends_on: B6 · джерело: `09 §3` · accept: підписка активується вебхуком, платні ендпоінти закриті без plan.
- **M9 · UI калорій** — бейдж ккал на картці страви + підсумок дня/тижня. depends_on: M6, B8.
- **M10 · UI заміни** — кнопка «↻ Замінити» + анімований перерахунок. depends_on: M6, B9.
- **M11 · Підписка/paywall** — екран планів, paywall-модалка на платній дії, стан підписки в профілі. depends_on: M1, B10 · джерело: `09 §3`.

Граф (додатково): B4→B8→M9 · B5→B9→M10 · B6→B10→M11.
